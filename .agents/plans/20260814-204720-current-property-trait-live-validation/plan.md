...
| --- | --- | --- |
| `docs/game/capability/get_entity_at_current_place.md` and current catalog fixture | Current Entity state is one combined Property/Trait page through `get_entity_at_current_place`; the standalone Property read is absent | The Property runner must use only the current catalog and retrieve the introduced Entity through the scoped read |
| `tools/agent-playtest` and token-free preflight on 2026-08-14 | Preflight passes, but the live runtime gate still expects removed `list_entity_property_at_current_place` | Currentize the runner, prompts, allowlists, schemas, fake proof and runtime gate without changing historical evidence |
| `tools/trait-playtest:999-1085`, `docs/evidence/trait.md` and token-free preflight on 2026-08-14 | Exact valid `Pip`/`Mara` outputs are rejected only by live `startswith("Pip ")`/`startswith("Mara ")` validators; the audited digest also drifted after accepted repository changes | Make fake/live validation share the exact-name contract, add regressions and freeze a fresh digest after all bound bytes settle |
| `docs/evidence/property.md` | Deterministic Property proof is complete; no paid invocation currently counts as Property evidence | A successful new current candidate may earn only the bounded initial-Property/observation claim |
| `docs/evidence/trait.md` | Deterministic Trait proof is complete; all retained paid candidates are exact failures before Trait mutation | Preserve those failures and record only the newly earned result |
...
- Reuse and currentize `tools/agent-playtest` for the bounded Property candidate;
...
| --- | --- | --- | --- |
| `tools/agent-playtest`, `tests/agent-playtest.sh`, `tools/agent-playtest-schema/` | Runnable fake/preflight path but legacy live catalog/read assumptions | Use current catalog, four grounded Action reads, scoped Entity fetch for observer Property proof and aligned strict validation | One Action submit, withheld selection/confirmation, no legacy tool, no retry, ownership-safe cleanup |
| `tools/trait-playtest`, `tests/trait-playtest.sh`, `tools/trait-playtest-schema/live-candidate.sha256` | Current scenario with live-only exact-name rejection and stale digest | Unify exact-name validators, add regression and freeze all settled bound bytes | Seven calls maximum, zero retries, stable Trait id, current protocol only |
| `.aicadia-playtest/`, `.aicadia-trait-playtest/` | Ignored retained evidence roots | Add at most one new candidate per runner after GO preflights | Directories 700, files 600, historical artifacts unchanged, exact ownership before drop |
| `docs/evidence/`, `docs/concept/log/2026-08.md` | Accurately reports no paid Property proof and failed Trait validation | Record proposed boundary, then exact earned results once | One home per truth; no success claim before success |
| `.agents/backlog/` and this plan | Investigation/discovery is the sole `Now / Proposed`; no plan is Active | On acceptance publish validation as `Now / Active` and move discovery to `Next / Proposed`; restore discovery to `Now / Proposed` after validation closes unless the User redirects | At most one current edge; both draft plans keep their distinct authority |
| `docs/game/`, World/schema/adapters | Delivered current contract | No change | Property/Trait semantics and parity remain exact |
...
- `tests/agent-playtest.sh` — current Property controller happy/failure paths.
- `tests/trait-playtest.sh` — exact live-validator/fake-controller parity.
...
- `tests/agent-playtest.sh` and `tests/trait-playtest.sh` pass.
...
- Property command: `DATABASE_URL='postgres://localhost/postgres' tools/agent-playtest run --confirm-token-spend`.
- Trait command: `DATABASE_URL='postgres://localhost/postgres' tools/trait-playtest run --confirm-token-spend --candidate-digest '<fresh-digest>'`.
