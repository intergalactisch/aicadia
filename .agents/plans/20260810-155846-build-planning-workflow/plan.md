...
- replace `docs/game/` as the executable game authority;
...
```text
.agents/plans/<YYYYMMDD-HHMMSS>-<short-kebab-slug>/plan.md
```
...
5. Record the accepted development-method choice once in
   `docs/concept/log/log.md`; do not change `docs/game/` or select a backlog item.
...
| P1 | completed | P0 | with P2 | Install the compact always-on rule and decision trail | `AGENTS.md`, `docs/concept/log/log.md` | rule and log agree; no game contract or backlog change |
...
in-scope refinement and points to `.agents/plans/`. Record the accepted method and
