...
(`src/agent_contract/instruction.md`, 2,278 words of wall-text under ALL-CAPS
headings) and the thirteen tool descriptions (`src/agent_contract/tool/*.md`) —
is rewritten for comprehension: natural language, real Markdown structure,
short sentences, consistent `CONTEXT.md` vocabulary, and a fixed per-tool
...
4. **Consistent vocabulary.** Terms come from `CONTEXT.md`; an internal term
...
5. **Deliberate redundancy, nothing else.** Each mutating tool restates only
   ...
   in the contract. The finalized set is recorded in `docs/game/agent.md`'s
   text-layering rule.
...
| --- | --- | --- |
| `src/agent_contract/instruction.md` | One 52-line file, fifteen ALL-CAPS blocks, 2,278 words; served byte-exact via `server/discover` (baseline-verified 2026-08-14) | Rewritten as Markdown section files; assembly becomes the served text |
| `src/agent_contract/tool/*.md` | Thirteen descriptions, 38–179 words each, heavy cross-repetition | Rewritten to the per-tool template with the deliberate boundary set |
| `src/agent_contract.rs` | Pinned-phrase tests assert long literal sentences; `apply()` strips one trailing newline per description | Pins redesigned to compact boundary phrases; assembly added; startup assert kept |
| `tests/agent-tool-catalog.json` | Byte truth of the published catalog | Regenerated exactly once in R3 and committed as a reviewed, deliberate contract change |
...
| `tests/aicadia-local.sh:212` | Compares the adapter's `developer_instructions=` argument against the disk file | Compares against the server-served instructions after R4 |
| `tools/aicadia-agent` | Guarded disk read of the contract; baseline green | One `server/discover` fetch with the exact stateless headers/`_meta` shape; `jq -ec '.result.instructions'` (never decode-and-re-encode); fail-closed at the same early position |
| `docs/game/agent.md` text-layering rule | Names today's deliberate per-tool redundancies | Updated in the same change to the finalized boundary set |
| `docs/game/capability/*.md` | Each carries an "MCP publication" annotation summary | Any restated description wording is aligned in R5 |
| `tools/trait-playtest` predicate | Candidate material now includes `*.md` under `src/` | Rewritten text is future freeze material; no pending candidate exists, so drift blocks nothing |
...
Seams: `src/agent_contract/**` (sections, tools, assembly), `src/agent_contract.rs`
(pins, assembly, narrow public seam), `src/lib.rs` (seam visibility),
`tests/server/` (comparison source), `tests/agent-tool-catalog.json`
(regeneration), `tools/aicadia-agent` + `tests/aicadia-local.sh` (discover
fetch), `docs/game/agent.md` + `docs/game/capability/*.md` (layering rule and
...
| `src/agent_contract/instruction/` | absent (one `instruction.md`) | One Markdown file per section, rewritten per the editorial rules; explicit ordered assembly | Every inventory rule lands exactly once; served text = assembly |
| `src/agent_contract/instruction.md` | the served contract | Removed after the assembly serves the rewritten text | Removal and seam switch in one task |
| `src/agent_contract/tool/*.md` | thirteen relocated originals | Rewritten to the per-tool template with the deliberate boundary set | One file per tool; names unchanged |
| `src/agent_contract.rs` | relocation-era pins; per-tool includes; newline strip | Assembly for sections; redesigned pin tests (compact boundary phrases per text); startup completeness assert kept | Pins cover every non-negotiable boundary named in the inventory |
...
| `tests/agent-tool-catalog.json` | relocation-era bytes | Regenerated once; reviewed as the deliberate contract change | Only `description` fields differ from the previous fixture |
...
| `tests/aicadia-local.sh` | compares against disk file | Compares recorded `developer_instructions=` against the served instructions | Test still proves byte-exact injection and fail-closed startup |
| `docs/game/agent.md` | layering rule names current redundancies | Finalized deliberate boundary set recorded; host paragraph updated for single delivery | Layering rule stays the editorial authority for future text changes |
| `docs/game/local-play.md` | describes disk-read adapter | Describes the discover-fetch delivery | Fail-closed meaning unchanged |
| `docs/game/capability/*.md` | publication lines from relocation | Aligned where they restate description wording | Capability semantics untouched |
...
| R2 | completed | R1 | yes | Contract rewrite as section files + assembly + seam + new contract pins | `src/agent_contract/instruction/**`, `instruction.md` removal, `src/agent_contract.rs` (instruction side), `src/lib.rs`, `tests/server/` comparison | `cargo test` green; served text = assembly; inventory items for the contract all ticked |
| R3 | completed | R1 | yes | Thirteen tool descriptions rewritten + fixture regeneration + new tool pins | `src/agent_contract/tool/*.md`, `src/agent_contract.rs` (tool side), `tests/agent-tool-catalog.json` | `cargo test` green; fixture diff touches only `description` fields; inventory items for tools all ticked |
| R4 | completed | R2 | no | Adapter single delivery via discover | `tools/aicadia-agent`, `tests/aicadia-local.sh` | Suite passes; stopped-server run fails closed; injected bytes equal served bytes |
| R5 | completed | R2, R3 | no | Documentation alignment + log entries | `docs/game/agent.md`, `docs/game/local-play.md`, `docs/game/capability/*.md`, concept log | Layering rule matches shipped redundancy; no stale wording |
...
1. Write `src/agent_contract/instruction/<nn>-<section>.md` per R1's
...
2. Replace the tool-side pinned tests with R1's set; regenerate
   `tests/agent-tool-catalog.json` once and review its diff: only
...
**Objective:** `tools/aicadia-agent` injects exactly the served instructions,
...
1. Replace the disk read with the `server/discover` POST described in the
   ...
   today's file guard.
2. Repoint `tests/aicadia-local.sh` to compare against the served
   instructions.
...
- `tests/aicadia-local.sh` green; manual stopped-server run fails closed with
  a clear message.
...
1. Record the finalized deliberate-redundancy set in `docs/game/agent.md` and
   update its host-delivery paragraph; update `docs/game/local-play.md`.
...
2. **Contract:** `cargo test --all-targets` green; `tests/aicadia-local.sh`,
   `tests/agent-playtest.sh`, `tests/trait-playtest.sh` green; live
...
`src/wire/output.rs`). Focused re-review returned **GO**: semantic parity
...
`description` fields; `tests/aicadia-local.sh` green including the new
served-instructions comparison; `tests/agent-playtest.sh` green;
`tests/trait-playtest.sh` green for its advertised token-free contract (no
...
(`tools/trait-playtest:100`) was updated to the new description phrasing in
