...
| `vocabulary` | development | `CONTEXT.md` | no | required | none |
| `game-index` | runtime | `docs/game/README.md` | no | required | none |
| `model` | runtime | `docs/game/model/<model>/README.md` | no | required | `model` vocabulary |
| `model-concern` | runtime | `docs/game/model/<model>/<other>.md` | no | required | none |
| `capability` | runtime | `docs/game/capability/<name>.md` | no | required | none |
| `game-contract` | runtime | `docs/game/<other>.md` | no | required | none |
| `concept-index` | development | `docs/concept/README.md` | no | required | none |
| `concept-record` | development | `docs/concept/<other>.md` | no | required | `concept` vocabulary |
| `concept-log-index` | development | `docs/concept/log/README.md` | no | required | none |
| `decision-register` | development | `docs/concept/log/<YYYY-MM>.md` | yes (links) | required | none |
| `concept-archive` | development | `docs/concept/archive/**/*.md` | yes | not required | none |
| `research-index` | development | `docs/research/README.md` | no | required | none |
| `research-report` | development | `docs/research/<other>.md` | no | required | `research` vocabulary |
| `research-archive` | development | `docs/research/archive/**/*.md` | yes | not required | none |
| `evidence-index` | bridge | `docs/evidence/README.md` | no | required | none |
| `evidence-slice` | bridge | `docs/evidence/<other>.md` | no | required | `evidence` vocabulary |
| `runner-contract` | bridge | `docs/evidence/runner/<name>.md` | no | required | none |
| `backlog-index` | development | `.agents/backlog/README.md` | no | required | none |
| `capability-map` | development | `.agents/backlog/capability-map.md` | no | required | none |
| `backlog-item` | development | `.agents/backlog/items/<name>.md` | no | required | none (state lives in the horizon table) |
| `plan` | development | `.agents/plans/<dir>/plan.md` | yes when `status` is `complete`, `superseded` or `dropped` | required unless frozen | `plan` vocabulary |
| `plan-fragment` | development | `.agents/plans/<dir>/**/<other>.md` | inherits its plan | not required | none |
| `skill` | development | `.agents/skills/<name>/SKILL.md` | no | required | skill front matter (`name`, `description`) — not validated beyond presence |
| `skill-asset` | development | `.agents/skills/<name>/**/<other>.md` | no | not required | not validated (templates contain placeholders) |
...
| `agent-contract-index` | runtime | `src/agent_contract/README.md` | no | required | none |
| `agent-instruction` | runtime | `src/agent_contract/instruction/<nn>-<name>.md` | no | not required (published bytes) | none |
| `agent-tool-text` | runtime | `src/agent_contract/tool/<name>.md` | no | not required (published bytes) | none |
...
A monthly file `docs/concept/log/YYYY-MM.md` is parsed as:
...
- Backlog horizon: the first table in `.agents/backlog/README.md` whose header row
...
- `docs/game/model/<model>/README.md` — title `# <Model name>`, role header, front
...
- Vocabulary join: the `CONTEXT.md` term whose bold name, lower-cased and
