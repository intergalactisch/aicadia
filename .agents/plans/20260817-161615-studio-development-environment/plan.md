...
updated_at: "2026-08-17T20:35:00+02:00"
...
| T7 | completed | — | with T5, T6 | Live reads: chronicles, full Entity/Place/Character/User/investigation/Property-key/Trait reads, resolver, estimates, migrations, row viewer (loopback game-read removal moved to T10). | `src/studio/live/*.rs`, `tests/studio/live.rs` | Completed 2026-08-17: 31 pure bounded reads in `src/studio/live/`, 16 disposable-database tests (bounds, keyset stability, D6 label, resolver, estimates, migrations, row viewer over all 14 tables incl. rejection of unknown/quoted names); EXPLAIN on a 200k-row throwaway database showed index-only scans for every indexed read and the labeled seq-scan only for the World chronicle. |
