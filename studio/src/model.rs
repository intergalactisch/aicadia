...
/// One canonical term of `dev/CONTEXT.md`.
...
pub fn vocabulary(repository: &Repository) -> Vec<Term> {
    let Some(record) = repository.get("dev/CONTEXT.md") else {
        return Vec::new();
    };
...
/// Every model folder under `game/docs/model/`, in id order.
///
/// An absent `game/docs/model/` directory yields an empty list rather than an error.
...
///
/// Returns `None` while `game/docs/model/<id>/README.md` does not exist.
pub fn contract(repository_root: &std::path::Path, id: &str) -> Option<(FrontMatter, String)> {
    let path = format!("game/docs/model/{id}/README.md");
...
pub fn lint_storage_table(repository: &Repository) -> Vec<LintFinding> {
    ...
    for (claimed, owner) in &claim {
        if owner.len() > 1 {
            finding.push(LintFinding {
                path: format!("game/docs/model/{}/README.md", owner[0]),
                line: Some(1),
                rule: RULE_STORAGE_TABLE,
    ...
    for table in &table {
        if !claim.contains_key(table.name.as_str()) {
            finding.push(LintFinding {
                path: format!("game/migration/{}", table.created_by),
                line: None,
                rule: RULE_STORAGE_TABLE,
...
mod tests {
    ...
    fn repository(file: &[(&str, &str)]) -> Repository {
        let root = crate::test_directory("model-fixture");
    ...
    fn model_files() -> Vec<(&'static str, &'static str)> {
        vec![
            ("dev/CONTEXT.md", CONTEXT),
            ("game/migration/0001_world.sql", MIGRATION_ONE),
            ("game/migration/0002_rename_app_user.sql", MIGRATION_TWO),
            ("game/migration/0010_investigation.sql", MIGRATION_THREE),
            (
                "game/docs/model/user/README.md",
                "---\nkind: subject\nstorage_table: [user]\n---\n\n# User\n\n> **Role / side:** User model contract / runtime side.\n> **Authority:** the durable participant.\n",
            ),
            (
                "game/docs/model/user/provenance.md",
            ...
            (
                "game/docs/model/investigation-attempt/README.md",
    ...
    fn a_model_folder_joins_its_term_concern_pages_tables_and_backlinks() {
        ...
        assert_eq!(user.concern, vec!["game/docs/model/user/provenance.md"]);
        assert_eq!(
    ...
    fn every_migration_table_must_be_claimed_by_exactly_one_model() {
        ...
        file.retain(|(path, _)| *path != "game/docs/model/entity/README.md");
        file.push((
            "game/docs/model/place/README.md",
        ...
        assert!(message.contains(&(
            "game/docs/model/place/README.md".to_owned(),
            "claimed table `place` is created by no migration".to_owned()
        )));
        assert!(message.contains(&(
            "game/migration/0001_world.sql".to_owned(),
    ...
    fn a_complete_model_set_lints_clean_and_an_absent_model_home_is_one_finding() {
        ...
        assert_eq!(absent_home[0].path, "game/docs/model");
