...
use crate::{StudioState, model};

#[derive(Debug, Deserialize)]
pub(crate) struct JumpQuery {
    #[serde(default)]
    q: String,
}
...
fn catalog(context: &Context) -> Vec<Target> {
    ...
    for model in model::models(repository) {
        target.push(Target {
            ...
            href: format!("/game/model/{}", model.id),
            kind: "model",
        });
    }
    for tool in &context.surface().tool {
        target.push(Target {
            ...
            href: format!("/game/capability/{}", tool.name),
            kind: "capability",
        });
    }
    for term in model::vocabulary(repository) {
        target.push(Target {
            ...
            href: format!("/game/vocabulary#{}", term.id),
            kind: "term",
        });
    }
...
#[cfg(test)]
fn repository_target(repository: &crate::record::Repository) -> Vec<Target> {
    repository
        .record()
        .iter()
        .map(|record| Target {
...
mod tests {
    ...
    fn target() -> Vec<Target> {
        vec![
            Target {
                ...
                href: "/game/model/entity".to_owned(),
                kind: "model",
                key: vec!["entity".to_owned()],
            },
            Target {
                label: "create_entity".to_owned(),
                href: "/doc/game/docs/capability/create_entity.md".to_owned(),
                kind: "capability",
                key: vec!["create_entity".to_owned()],
            },
            Target {
                label: "Entity state rationale".to_owned(),
                href: "/doc/dev/docs/concept/entity-state.md".to_owned(),
                kind: "concept record",
                key: vec![
                    "dev/docs/concept/entity-state.md".to_owned(),
    ...
    fn an_exact_name_wins_over_every_prefix() {
        assert!(matches!(
            ...
            Resolution::Exact(href) if href == "/game/model/entity"
        ));
    }
    ...
    fn a_unique_prefix_resolves_and_a_shared_one_lists() {
        ...
        assert!(matches!(
            ...
            Resolution::Exact(href) if href == "/doc/dev/docs/concept/entity-state.md"
        ));
        assert!(matches!(
            resolve(&target(), "ent"),
    ...
    fn every_governed_record_path_resolves_exactly() {
        let repository = crate::record::Repository::load(crate::workspace_root())
