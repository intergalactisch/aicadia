...
use crate::{
    StudioState,
    record::{Kind, LinkTarget, Record, Repository, resolve_link},
};
...
/// Only variable status, verdict and frozen state belong above the page title.
...
mod tests {
    ...
    fn repository() -> Repository {
        Repository::load(crate::workspace_root()).expect("the governed roots parse")
    }
    ...
    fn a_record_with_sections_gets_its_anchors_and_studio_links() {
        ...
        assert!(pointer_html.contains("href=\"/doc/dev/docs/README.md\""));
    }
    ...
    fn a_migration_renders_as_sql_code() {
        ...
        let record = repository
            .get("game/migration/0001_world.sql")
    ...
    fn a_link_outside_the_governed_roots_is_disabled_rather_than_broken() {
        ...
        assert!(matches!(
            rewrite(record, &repository, "game/src/world/mod.rs"),
            Route::Outside
        ));
        assert!(matches!(
            rewrite(record, &repository, "dev/docs/README.md#homes"),
            Route::Studio(_)
        ));
    }
}
