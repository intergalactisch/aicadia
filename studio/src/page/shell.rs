                ...
                div class="frame" {
                    ...
                    main class="main" id="content" tabindex="-1" {
                        ...
                        @if let Some(reference) = &page.reference {
                            @if !plate_copies_reference(page, reference) { (colophon(reference)) }
                        }
                    }
                }
...
fn plate_copies_reference(page: &Page, reference: &Reference) -> bool {
    let line = reference.line();
    page.plate.as_ref().is_some_and(|plate| {
        plate
            .action
            .iter()
            .any(|action| matches!(action, Action::Copy { text, .. } if text == &line))
    })
}
...
fn bar(context: &Context, current: Section) -> Markup {
    ...
    html! {
        header class="bar" {
            ...
            a class="mark" href="/" aria-label="Homepage" {
            ...
            div class="bar-end" {
                form class="jump" role="search" action="/jump" method="get"
                    ...
                    input type="search" name="q" autocomplete="off"
                        placeholder="Jump to model, tool, record, table…"
                        aria-label="Jump to";
                    kbd { "/" }
                }
