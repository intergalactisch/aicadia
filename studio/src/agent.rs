...
pub fn surface(repository: &Repository, catalog: Vec<Tool>) -> AgentSurface {
    let section = aicadia::agent_contract::instruction_section()
    ...
    let tool = catalog
        ...
        .map(|tool| {
            ...
            let capability_path = format!("game/docs/capability/{name}.md");
            let text_path = format!("game/mcp/agent/tool/{name}.md");
            let route = operation
                .iter()
    ...
    AgentSurface {
        section,
        instructions: aicadia::agent_contract::instructions(),
        tool,
    }
}
...
fn http_operation() -> Vec<(String, &'static str, String)> {
    let document = aicadia::server::openapi_document();
...
mod tests {
    ...
    fn fixture() -> Repository {
        let root = crate::test_directory("agent-fixture");
        for path in [
            "game/docs/capability/get_world.md",
            "game/mcp/agent/tool/get_world.md",
        ] {
    ...
    fn catalog() -> Vec<Tool> {
        ...
        aicadia::server::mcp_tool_catalog(crate::World::new(pool))
    }
        ...
        assert!(
            surface
                ...
                .all(|section| !section.title.is_empty()
                    && section.path.starts_with("game/mcp/agent/instruction/"))
        );
        assert_eq!(
            surface.instructions,
            aicadia::agent_contract::instructions(),
