...
/// The keyset of one User's attempt page in its existing unique-index order.
...
/// stored `created_at`. For one User's index-bounded attempts, use
/// [`list_user_attempt`].
...
/// One User's investigation attempts in request-id order.
///
/// Served by the unique
/// `investigation_attempt_requested_by_user_id_request_id_key` index —
/// `(requested_by_user_id, request_id)`. A request id is a random UUID, so this
/// is deliberately not presented as chronological order; every row still carries
...
pub async fn list_user_attempt(
    ...
    let item = sqlx::query_as::<_, AttemptItem>(
        ...
        WHERE attempt.requested_by_user_id = $1
          AND ($2::uuid IS NULL OR attempt.request_id > $2::uuid)
        ORDER BY attempt.request_id
        LIMIT $3
        "#,
    )
    .bind(user_id)
    .bind(before.map(|cursor| cursor.request_id))
    .bind(bound.fetch())
    .fetch_all(pool)
    .await?;
...
///
/// Identity, the Character and Place Entities, the consuming Activity and the
/// forward `voided_by_attempt_id` link are primary-key lookups. The reverse
/// direction has no leading index and is deliberately absent.
