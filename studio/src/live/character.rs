...
/// One Character's investigation attempts in request-id order.
///
...
/// through the unique `(requested_by_user_id, request_id)` index. The two sets
/// are the same set: a User owns at most one Character, and every attempt stores
/// the Character World derived from its requesting User.
