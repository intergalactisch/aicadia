...
readonly LAUNCHER="$REPO_DIR/studio/tools/aicadia-local"
readonly AGENT="$REPO_DIR/game/tools/aicadia-agent"
...
first_user_id="$(sed -n "s/^AICADIA_USER_ID='\([^']*\)' AICADIA_PORT='$PORT' \.\/game\/tools\/aicadia-agent$/\1/p" "$first_stdout")"
...
second_user_id="$(sed -n "s/^AICADIA_USER_ID='\([^']*\)' AICADIA_PORT='$PORT' \.\/game\/tools\/aicadia-agent$/\1/p" "$second_stdout")"
...
grep -Fx '/dev/.local/' "$REPO_DIR/.gitignore" >/dev/null \
