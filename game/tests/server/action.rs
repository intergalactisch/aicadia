...
async fn http_and_mcp_share_successful_world_state(pool: PgPool) {
    ...
    let character_entity_id = Uuid::parse_str(
        http_character["entity"]["id"]
            .as_str()
            .expect("Character Entity id should be text"),
    )
    .expect("Character Entity id should be a UUID");
    let character_entity = sqlx::query_as::<_, (Uuid, String, String)>(
        "SELECT id, name, description FROM entity WHERE id = $1",
    )
    .bind(character_entity_id)
    .fetch_one(&pool)
    .await
    .expect("authoritative Character Entity readback should succeed");
    assert_eq!(character_entity.0, character_entity_id);
    assert_eq!(character_entity.1, http_character["entity"]["name"]);
    assert_eq!(character_entity.2, http_character["entity"]["description"]);
    ...
    let http_created_id = Uuid::parse_str(
        http_created["id"]
            .as_str()
            .expect("Entity id should be text"),
    )
    .expect("created Entity id should be a UUID");
    let operator_read = sqlx::query_as::<_, (Uuid, String, String)>(
        "SELECT id, name, description FROM entity WHERE id = $1",
    )
    .bind(http_created_id)
    .fetch_one(&pool)
    .await
    .expect("authoritative Entity readback should succeed");
    assert_eq!(operator_read.0, http_created_id);
    assert_eq!(operator_read.1, http_created["name"]);
    assert_eq!(operator_read.2, http_created["description"]);
    ...
    let mcp_created_id = Uuid::parse_str(
        mcp_created["id"]
            .as_str()
            .expect("MCP Entity id should be text"),
    )
    .expect("MCP Entity id should be a UUID");
    let mcp_readback: Uuid = sqlx::query_scalar("SELECT id FROM entity WHERE id = $1")
        .bind(mcp_created_id)
        .fetch_one(&pool)
        .await
        .expect("MCP-created Entity should have authoritative readback");
    assert_eq!(mcp_readback, mcp_created_id);
}
