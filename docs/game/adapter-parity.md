...
9. All five Entity-creation routes share the same independent optional 0–100 initial
...
13. All creation routes establish optional 0–100 initial Traits; mixed 0–100 Action
    establishment/development and optional 0–100 actor/target Interaction Trait
    changes preserve stable ids, immutable roots/versions/current pointers, exact
    Activity history, Property coexistence and atomic rollback. Deferred bounded
    per-Trait commit checks enforce exactly one root, exactly one current pointer and
    a pointer at the lineage tip, rejecting incomplete root/pointer state, current
    deletion or backtracking and successor-without-advance.
