...
Entity-associated current state now execute under the active
...
ownership-verified cleanup `dropped`; sixteen fake failure injections pass. No paid
candidate or Trait model run has occurred. T9 still requires fresh explicit User
authorization of the exact digest-bound command published in the linked dedicated
contract. Global `list_entity` and `get_entity` remain loopback operator/ledger reads
outside player knowledge and MCP.
