# Local play and ledger evidence

> **Role / side:** local launcher, Agent handoff and ledger delivery history / evidence bridge.
> **Authority:** owns the retained deterministic completion record for persistent local play and the read-only ledger.
> **Excludes:** current local-operation and ledger contracts; see `docs/game/local-play.md`.
