...
`docs/game/`. Aicadia remains one deterministic `World` over PostgreSQL with six
...
stays in `docs/game/`. Research stays in `docs/research/`. A future skill may encode
...
recommendation never overrules `AGENTS.md`, `docs/game/`, executable behavior or the
user.
...
- **Document gate:** research may inform a proposal but cannot change behavior.
  ...
  `docs/game/` by the designated writer as part of the implementation change.
...
- **Specification review:** does the diff implement exactly the requested behavior
  and current `docs/game/` contract, including HTTP/MCP parity where applicable?
...
4. Once the manual workflow is stable across two or three concrete use cases, refine
   the existing `.agents/skills/build-aicadia/SKILL.md` with only the graph, work
