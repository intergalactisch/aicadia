...
> **Authority:** identifies each concept record's theme and where to read it.
> **Excludes:** Each record's own exploration status, which lives in its front matter; current implementation and game behavior, which belong in `docs/game/`; delivery evidence, which belongs in `docs/evidence/`.
...
only when the owning `docs/game/` contract explicitly accepts it. Each record carries
its own exploration status in its front matter.

## Themed records

- [Discovery and investigation](discovery.md) — investigation, rolls, volatile transport, meta-state handoff, drill-down, provenance, open frontier and prototype.
...
- [Interaction participation rationale](interaction.md) — asymmetric participation; delivered behavior is a pointer to `docs/game/`.
