...
- accepted next edge / plan concurrent Entity requests before deliberation: the User
  accepted a new retained lab experiment in which simultaneous Agent-authored
  requests target one Tree through real PostgreSQL and MCP. Same exact facts must
  conflict or shed boundedly; different facts may compose; and explicit Agent-owned
  causal dependencies determine whether `color=blue` remains valid after a concrete
  `condition=felled` change. World infers no destruction, importance or lifecycle
  ontology. Voting, discussion windows and cooldown remain outside this first slice.
  The existing multiplayer plan returned to `draft` for the exact workload,
  real/simulated seams and a proposed maximum-three-call Agent comprehension tier;
  no experiment implementation or model call begins before explicit re-acceptance.
- accepted revised execution / concurrent Entity experiment may begin: the User
  explicitly re-accepted the detailed T3A–T3E plan and its maximum of three pinned
  Agent process calls, zero retries and no enforceable token ceiling. T3A starts with
  only the deterministic scratch-PostgreSQL kernel and Tree matrix. The Agent calls
  remain gated on green T3A and real MCP T3C evidence and must be announced directly
  before execution; failure is retained as evidence and never earns another call.
- corrected / exact facts are evidence, not yet the multiplayer foundation: after
  the first scratch-PostgreSQL matrix passed, the User challenged the premise that an
  Agent should identify every Property on which its action may semantically depend.
  An unmentioned concurrent change cannot be made safe by subscriptions alone, and
  a dumb World cannot infer its relevance. The active choice is now whole-Entity
  snapshot freshness versus explicitly narrower exact-fact freshness; the former
  preserves semantic reconsideration but creates one hot-Entity settlement lane,
  while the latter preserves concurrency but accepts unknown composition. The plan
  returned to `draft`, T3A is blocked as a foundation verdict, and no load, MCP or
  Agent-call tier proceeds before one-at-a-time grilling resolves this boundary.
- accepted Q9 / whole-Entity basis for an uncontested request: the User selected a
  coherent Entity snapshot rather than an Agent-curated subset of Property
  dependencies. The Agent interprets the content; World only validates the current
  basis. A changed Entity forces reread and reconsideration. The User added that a
  second genuinely concurrent request must not simply lose to arrival order; it
  should enter a bounded collective settlement path. What deterministic rule forms
  that concurrent set remains open, so the plan stays `draft` and experiment
  implementation remains paused.
- accepted architectural direction / one global participation-and-settlement system:
  the User rejected further mutation-path alternatives. An active Agent registers in
  its durable Character's current Place, registers a bounded intent before changing
  an Entity, discovers concurrent intent through that Place, and reaches Agent-owned
  consensus or voting inside a fixed timeframe before one deterministic World
  commit. A bounded trigger web extends declared effects to other Entity/Place
  scopes. MMO-style “instances” may inspire local coordination only; World state is
  never copied. Registration ownership and lifetime are the next one-at-a-time grill
  prerequisite, followed by trigger links and scalable settlement. No implementation
  resumes while the revised plan is `draft`.
