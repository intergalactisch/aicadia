...
pub(super) use section::{development, game, live, overview, stub, stub_path, stub_table};
...
impl PanelItem {
    ...
    /// A row that names a fact rather than a destination.
    pub(super) fn text(label: impl Into<String>) -> Self {
        Self {
            label: label.into(),
            href: None,
            note: None,
...
impl Panel {
    ...
    pub(super) fn definition(title: impl Into<String>, item: Vec<(String, String)>) -> Self {
        Self {
            title: title.into(),
            body: PanelBody::Definition(item),
        }
    }
}
...
impl Context {
    ...
    pub(super) async fn build(
        ...
        let surface = super::agent::surface(
            &repository,
            crate::server::mcp_tool_catalog(state.world.clone()),
        );
        let origin = header
            .get(header::HOST)
...
mod tests {
    ...
    fn a_reference_line_keeps_the_agreed_copy_format() {
        let reference = Reference {
            ...
            url: "http://127.0.0.1:3000/doc/docs/game/model/entity/README.md".to_owned(),
            context: "docs/game/model/entity/README.md".to_owned(),
        };

        assert_eq!(
            reference.line(),
            "[Aicadia Studio · Entity](http://127.0.0.1:3000/doc/docs/game/model/entity/README.md) — docs/game/model/entity/README.md"
        );
    }
