...
/// Every applied migration, joined by version to the files under `migration/`.
///
...
pub async fn list_migration(
    ...
    let mut file = repository_file(&repository_root.join("migration"))?;
    file.sort();
    let applied = row
        .into_iter()
        .take(MAX_MIGRATION)
...
mod tests {
    ...
    fn the_repository_migration_directory_lists_only_sql_files() {
        let file = repository_file(&Path::new(env!("CARGO_MANIFEST_DIR")).join("migration"))
