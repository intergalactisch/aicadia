...
/// The one chronicle projection. `$1` is the optional scope id, `$2` the optional
/// operation filter, `$3`/`$4` the keyset, `$5` the row bound, `$6` the prose
/// preview length and `$7` the bound on the involved-Entity count.
///
...
fn chronicle_sql(scope: &'static str) -> String {
    format!(
        ...
        SELECT activity.id,
               ...
               left(activity.prose, $6::integer) AS prose,
               COALESCE(char_length(activity.prose) > $6::integer, false) AS prose_truncated,
               (
                   SELECT count(*)
                   FROM (
                       SELECT 1
        ...
        WHERE {scope}
          AND ($2::text IS NULL OR activity.operation = $2::text)
          AND (
...
pub async fn list_world_chronicle(
    ...
    let page = read_chronicle(
        pool,
        chronicle_sql("($1::uuid IS NULL OR activity.id = $1::uuid)"),
        None,
        operation,
        before,
        bound,
