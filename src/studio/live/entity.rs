...
#[derive(Debug, Serialize, sqlx::FromRow)]
pub struct EstablishingActivity {
    pub id: Uuid,
...
pub struct EntityDetail {
    ...
    pub establishing_activity: Option<EstablishingActivity>,
...
/// One Entity with its roles, current Place, establishing Activity and a bounded
/// preview of its current Properties and Traits.
///
...
///
/// The establishing Activity is the oldest Activity in which this Entity appears
/// with role `subject`. That participation is indexed by
/// `activity_entity_entity_id_activity_id_index`, but no index orders it by
/// time, so this read sorts one Entity's own subject participation by the stored
/// `occurred_at`. The work is scoped to the single Entity and never to the
/// Activity table.
pub async fn get_entity(pool: &PgPool, entity_id: Uuid) -> Result<EntityDetail, StudioError> {
    ...
    let establishing_activity = sqlx::query_as::<_, EstablishingActivity>(
        r#"
        SELECT activity.id,
               activity.operation,
