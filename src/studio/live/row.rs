...
pub struct RowPage {
    ...
    pub truncated: bool,
    /// True when the table has no primary key and rows could only be ordered by
    /// physical position, which PostgreSQL may change at any time.
...
///
/// A table without a primary key has no stable order to key on. Studio then
/// orders by `ctid`, returns no cursor beyond the `LIMIT` and sets
/// `unstable_order`, rather than pretending the page can be resumed.
///
/// Every current Aicadia table has a primary key; the `ctid` path exists so a
/// future one without a key degrades honestly instead of silently repeating or
/// skipping rows.
pub async fn list_row(
    ...
    let cursor_projection = if key_column.is_empty() {
        "ARRAY[]::text[]".to_owned()
    } else {
    ...
    let where_clause = match after {
        Some(after) if !key_column.is_empty() && after.len() == key_column.len() => {
            let left = key_column
                .iter()
    ...
    let order_clause = if key_column.is_empty() {
        format!("ORDER BY {ROW_ALIAS}.ctid")
    } else {
    ...
    let next_cursor = match (truncated, key_column.is_empty(), fetched.last()) {
        (true, false, Some((_, cursor))) => Some(cursor.clone()),
        _ => None,
    };
    let row = fetched
        .into_iter()
    ...
    Ok(RowPage {
        ...
        unstable_order: primary_key.is_empty(),
        primary_key,
        row,
        next_cursor,
        truncated,
