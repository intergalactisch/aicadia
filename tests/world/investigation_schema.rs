...
#[sqlx::test]
async fn rolling_hour_predicate_is_inclusive_at_exact_database_boundary(pool: PgPool) {
    let admitted: Vec<bool> = sqlx::query_scalar(
        r#"
        WITH fixed AS MATERIALIZED (
            SELECT statement_timestamp() AS database_now
        ), sample(created_at) AS (
            SELECT database_now - interval '1 hour' FROM fixed
            UNION ALL
            SELECT database_now - interval '1 hour 1 microsecond' FROM fixed
        )
        SELECT created_at >= database_now - interval '1 hour'
           AND created_at <= database_now
        FROM sample CROSS JOIN fixed
...
async fn investigation_indexes_support_bounded_hot_subject_queries(pool: PgPool) {
    ...
    let user_id = create_user(&world).await;
    let character = world
        .create_character(user_id, character("Plan actor"))
        .await
        .unwrap();
    let place = world
        .create_entry_place(user_id, place("Plan place"))
        .await
        .unwrap();
    world.enter_world(user_id).await.unwrap();
    ...
    sqlx::query("SET LOCAL enable_seqscan = off")
        ...
        .await
        .unwrap();
    let activity_plan = sqlx::query_scalar::<_, String>(
        r#"
        EXPLAIN (COSTS OFF)
        SELECT operation
        FROM activity
    ...
    .join("\n");
    assert!(
        activity_plan.contains("activity_place_occurred_at_id_index"),
        "{activity_plan}"
    );
    let admission_plan = sqlx::query_scalar::<_, String>(
        r#"
        EXPLAIN (COSTS OFF)
        SELECT count(*)
    ...
    .join("\n");
    assert!(
        live_plan.contains("investigation_attempt_live_positive_index"),
        "{live_plan}"
    );
}
