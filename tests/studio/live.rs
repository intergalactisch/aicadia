...
use aicadia::studio::StudioError;
use aicadia::studio::live::{
...
async fn every_live_list_stops_at_its_bound_and_names_the_row_after_it(pool: PgPool) {
    ...
    let world_chronicle = chronicle::list_world_chronicle(&pool, None, None, bound)
        .await
...
async fn every_chronicle_is_newest_first_and_the_world_chronicle_says_what_it_is(pool: PgPool) {
    ...
    let place_chronicle =
        chronicle::list_place_chronicle(&pool, seed.place_entity_id, None, None, bound)
            .await
    ...
    let character_chronicle =
        chronicle::list_character_chronicle(&pool, seed.character_entity_id, None, None, bound)
            .await
    ...
    // The optional operation filter narrows a chronicle without changing its order.
    let filtered = chronicle::list_world_chronicle(&pool, Some("create_entity"), None, bound)
        .await
        .expect("the filtered World chronicle reads");
    assert_eq!(filtered.page.item.len(), 2);
    assert!(
        filtered
            .page
            .item
            .iter()
            .all(|item| item.operation == "create_entity")
    );
...
#[sqlx::test(migrations = "./migration")]
async fn entity_detail_joins_roles_state_history_and_the_activity_that_established_it(
    pool: PgPool,
) {
    let seed = seed(&pool).await;
    ...
    let establishing = detail
        .establishing_activity
        .as_ref()
        .expect("an introduced Entity has an establishing Activity");
    assert_eq!(establishing.operation, "submit_action");
    assert_ne!(establishing.id, seed.change_activity_id);
...
async fn the_resolver_answers_every_kind_of_exact_id_and_stays_silent_otherwise(pool: PgPool) {
    ...
    for (id, subject) in [
        ...
        (seed.change_request_id, "activity request"),
        (seed.attempt_request_id, "investigation attempt request"),
    ] {
    ...
    assert_eq!(resolved.request_id_scope, "local development scan");
...
async fn applied_migrations_match_the_repository_migration_files(pool: PgPool) {
    ...
    let file_count = std::fs::read_dir(root.join("migration"))
...
async fn place_character_user_and_investigation_reads_cross_link_their_subjects(pool: PgPool) {
    ...
    assert!(
        ...
        "one User's attempts are newest-first"
    );
    ...
    assert_eq!(
        attempt_detail.voided_attempt_scope,
        "local development scan"
    );
    assert!(!attempt_detail.voided_attempt_truncated);
...
#[sqlx::test(migrations = "./migration")]
async fn property_key_reads_name_their_type_first_use_and_bounded_holders(pool: PgPool) {
    let seed = seed(&pool).await;
    let bound = Bound::new(Some(100)).expect("the maximum bound");
    ...
    assert_eq!(detail.holder.len(), 3);
    assert!(!detail.holder_truncated);
    assert_eq!(detail.holder_scope, "local development scan");
    let changed = detail
        .holder
        .iter()
        .find(|holder| holder.entity_id == seed.changed_entity_id)
        .expect("the changed Entity still holds the key");
    assert_eq!(changed.integer_value, Some(64));
    assert_eq!(changed.current_activity_id, seed.change_activity_id);
...
async fn a_hundred_and_one_rows_page_as_one_hundred_rows_and_a_cursor(pool: PgPool) {
    ...
    let chronicle = chronicle::list_world_chronicle(&pool, None, None, bound)
        .await
        .expect("the World chronicle reads");
    assert_eq!(chronicle.page.item.len(), 100);
    assert!(chronicle.page.truncated);
    let last = chronicle::list_world_chronicle(&pool, None, chronicle.page.next_cursor, bound)
        .await
