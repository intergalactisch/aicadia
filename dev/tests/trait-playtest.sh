...
readonly RUNNER="$REPO_DIR/dev/playtest/trait/run"
...
copy_candidate_material() {
    local copy="$1"
    mkdir -p "$copy/game/mcp/agent" "$copy/game/migration" "$copy/game/src" \
        "$copy/dev/src" "$copy/dev/playtest/trait/schema" "$copy/studio/src"
    cp "$RUNNER" "$copy/dev/playtest/trait/run"
    cp "$REPO_DIR/Cargo.toml" "$REPO_DIR/Cargo.lock" "$copy/"
    cp "$REPO_DIR/game/Cargo.toml" "$copy/game/"
    cp "$REPO_DIR/dev/Cargo.toml" "$copy/dev/"
    cp "$REPO_DIR/studio/Cargo.toml" "$copy/studio/"
    cp "$REPO_DIR/game/mcp/tool-catalog.json" "$copy/game/mcp/"
    cp -R "$REPO_DIR/game/mcp/agent/." "$copy/game/mcp/agent/"
    cp -R "$REPO_DIR/game/src/." "$copy/game/src/"
    cp -R "$REPO_DIR/game/migration/." "$copy/game/migration/"
    cp -R "$REPO_DIR/dev/src/." "$copy/dev/src/"
    cp -R "$REPO_DIR/studio/src/." "$copy/studio/src/"
    cp "$REPO_DIR/dev/playtest/trait/schema/"* "$copy/dev/playtest/trait/schema/"
    chmod +x "$copy/dev/playtest/trait/run"
}
...
test_schema_policy_fails_before_evidence() {
    ...
    cp -R "$REPO_DIR/dev/playtest/trait/schema" "$schema_root"
...
test_live_gate_and_freeze() {
    ...
    mkdir -p "$root"
    digest="$(<"$REPO_DIR/dev/playtest/trait/schema/live-candidate.sha256")"
    before="$(candidate_state_snapshot "$REPO_DIR/dev/.playtest/trait")"
    ...
    after="$(candidate_state_snapshot "$REPO_DIR/dev/.playtest/trait")"
...
test_candidate_digest_binds_runtime_build_and_validator() {
    ...
    original="$("$copy/dev/playtest/trait/run" test-internal-candidate-digest --confirm-fake-controller-test)"
    printf '%s\n' "$original" >"$copy/dev/playtest/trait/schema/live-candidate.sha256"
    printf '\n// isolated digest drift\n' >>"$copy/game/src/world/mutation.rs"
    mutated="$("$copy/dev/playtest/trait/run" test-internal-candidate-digest --confirm-fake-controller-test)"
    [[ "$mutated" != "$original" ]] || fail 'bound World build-input mutation did not change candidate digest'
    before="$(candidate_state_snapshot "$copy/dev/.playtest/trait")"
    if (cd "$copy" && dev/playtest/trait/run run --confirm-token-spend --candidate-digest "$original") \
        >"$copy/old.stdout" 2>"$copy/old.stderr"; then
        fail 'old supplied digest passed after bound build-input drift'
    fi
    after="$(candidate_state_snapshot "$copy/dev/.playtest/trait")"
...
test_owned_preflight_cleanup_after_catalog_failure() {
    ...
    copy_candidate_material "$copy"
    mkdir "$copy/dev/.playtest/trait"
    chmod 700 "$copy/dev/.playtest/trait"
    ...
    find "$copy/dev/.playtest/trait" -maxdepth 2 -name manifest.json -path '*/preflight-*/*' -print 2>/dev/null | sort >"$before"
    if (cd "$copy" && PATH="$portable_bin:$PATH" CODEX_BIN=portable-codex \
        AICADIA_INTERNAL_TRAIT_PREFLIGHT_MODE=fail-after-catalog \
        dev/playtest/trait/run test-internal-live-preflight --confirm-fake-controller-test) \
    ...
    find "$copy/dev/.playtest/trait" -maxdepth 2 -name manifest.json -path '*/preflight-*/*' -print | sort >"$after"
...
test_public_cli_version_drift_fails_before_candidate() {
    ...
    copy_candidate_material "$copy"
    baseline="$("$copy/dev/playtest/trait/run" test-internal-candidate-digest --confirm-fake-controller-test)"
    printf '%s\n' "$baseline" >"$copy/dev/playtest/trait/schema/live-candidate.sha256"
    mkdir -p "$root/bin"
    ...
    if (cd "$copy" && PATH="$root/bin:$PATH" DATABASE_URL='postgres://must-not-be-used.invalid/postgres' \
        dev/playtest/trait/run preflight) >"$root/stdout" 2>"$root/stderr"; then
