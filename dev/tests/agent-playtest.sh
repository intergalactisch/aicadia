...
readonly RUNNER="$REPO_DIR/dev/playtest/agent/run"
...
make_fakes() {
    ...
    cp "$REPO_DIR/game/mcp/tool-catalog.json" "$root/state/catalog.json"
...
test_historical_property_catalog_snapshot() {
    local catalog="$REPO_DIR/dev/playtest/agent/schema/historical-agent-tool-catalog.json"
    jq -e '
        length == 13
...
test_forbidden_schema_keyword_fails_before_codex() {
    ...
    mkdir -p "$schema_dir"
    cp "$REPO_DIR"/dev/playtest/agent/schema/*.json "$schema_dir/"
