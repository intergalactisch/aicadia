...
> **Excludes:** each track's and experiment's own status and verdict, which live in their front matter; current game behavior, production implementation, sourced research authority and delivery proof; see `game/docs/`, runtime sources, `dev/docs/research/` and `dev/docs/evidence/`.

`dev/lab/` preserves small experiments that help Aicadia make a concrete product or
...
Current design rationale and open choices live in `dev/docs/concept/`, chronological
decisions in `dev/docs/concept/log/`, sourced findings in `dev/docs/research/`, and accepted
current behavior in `game/docs/`.
...
`cargo test --manifest-path dev/lab/<track>/<experiment>/Cargo.toml` so state,
