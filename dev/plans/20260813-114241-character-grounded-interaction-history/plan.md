...
| --- | --- | --- |
| `game/docs/README.md` | `submit_action` has one actor, one exact Place, one introduced Entity and immutable Activity/prose | Add one distinct Interaction operation behind the same deep `World` seam rather than disguise it as an Action consequence or create another service |
| `game/src/world.rs`, `activity`, `activity_entity` | Activity already supports one optional actor and many involved Entity rows, but roles are only `subject`, `destination`, `location` | Cardinality can reuse the seam; concrete participation and observation meanings still need a decision |
| `game/docs/agent-interface.md` | Agent reads current Place, offers three directions and obtains exact confirmation | Preserve private workshop and one irreversible commit |
| `dev/docs/concept/10-discovery-and-world-context.md` | Shared state is not universal knowledge; participation, observation and knowledge can be asymmetric while control provenance stays outside player mode | Scope reads and outputs by Character; never rely on narration to hide over-broad data |
| `dev/docs/research/dnd-rules-and-campaign-loops.md` | Durable play loops turn grounded situation and intent into authoritative consequence and a new decision | The first interaction must leave a truthful response opportunity, not a completed response |
| `dev/docs/research/dnd-campaign-meta-and-storytelling.md` | Recurring actors, callbacks and remembered consequences create campaign continuity | Interaction history must support later grounded recurrence without campaign schemas |
| `dev/docs/research/tabletop-narrative-meta-beyond-dnd.md` | Authority, stakes, observation and recap should not impersonate each other | Keep User intent, Agent composition, World result and Character knowledge separate |
| `dev/backlog/capability-map.md` | Current and future User capabilities need one living index | Update status and links with every accepted scope or completion |
...
typed checks live in `dev/docs/concept/11-entity-traits-and-change.md`.
...
follow-ons in `dev/docs/concept/11-entity-traits-and-change.md`. Their current relational
...
| --- | --- | --- | --- |
| `dev/docs/concept/10-discovery-and-world-context.md`, `dev/CONTEXT.md`, concept log | Knowledge, free-expression Interaction and roles explored and resolved | Preserve exact first-slice language and supersede stale alternatives | Concepts remain distinct; `game/docs/` remains current truth until build |
| `dev/backlog/README.md`, capability map, interaction item | Proposed current edge and full horizon | Keep one current status and exact evidence links | Backlog never becomes game authority |
| `game/docs/README.md`, `game/docs/agent-interface.md` | Thirteen capabilities, global Entity reads, marker-only action | Specify the accepted directed interaction and Character-scoped reads; narrow/reclassify global reads | Agent capability parity; natural player presentation; current-only contract |
...
| `game/src/world.rs` | One deep World seam; contextual exact-Place reads and marker commit | Add contextual directed-Entity selection/read, interaction acceptance and scoped history/knowledge | World derives User/Character/Place; atomicity; idempotency; freshness; no LLM |
| `game/src/wire.rs` | Current request/results expose global and exact-Place data | Encode one strict semantic contract with no User/control leakage | Deny unknown fields; no caller-supplied User/actor/Place |
| `game/src/server.rs`, `game/mcp/agent.rs`, `src/agent-play-contract.txt` | HTTP/MCP parity and private workshop for marker action | Ship matching interaction/read tools and grounded natural heuristics | Thin adapters; provider-neutral; unauthorized facts absent before model |
...
| T7 | completed | T5,T6 | no | Align current docs, capability map, backlog and exact bounded playtest evidence | `game/docs/`, `dev/backlog/`, concept log, plan, approved harness surfaces | Full validation ladder and the deterministic rat outcome support exactly the claim; no paid model run was used |
...
1. Update `game/docs/` with the new operation/read contract, errors, Activity roles,
...
4. Update `game/docs/`, capability map, backlog item, concept log and plan status to
   final truth.
...
- `dev/tests/agent-playtest.sh` — passed with fake Codex/curl/cargo only after aligning
...
- `cargo fmt --all -- --check`, strict all-target/all-feature Clippy, Bash syntax,
  ...
  passed.
- `DATABASE_URL=postgres://localhost/postgres dev/playtest/agent/run preflight` passed
...
- `PATH=/opt/homebrew/Cellar/postgresql@17/17.8/bin:$PATH
  DATABASE_URL=postgres://localhost/postgres studio/tests/aicadia-local.sh` passed using the
