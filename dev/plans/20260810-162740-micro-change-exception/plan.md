...
- change gameplay behavior, `game/docs/`, the backlog or the plan template;
...
| `dev/docs/concept/log/log.md` | Recorded method currently has no exception | Record the corrected boundary |
...
| `dev/skills/build-aicadia/SKILL.md` | Classify micro-changes before formal planning | Direct requests are not implicit formal-plan acceptance |
| `dev/skills/build-aicadia/agents/openai.yaml` | Make the prompt conditional | `$build-aicadia` named; implicit invocation preserved |
| `dev/docs/concept/log/log.md` | Record the corrected boundary | No duplicated implementation detail |
...
1. Run the skill creator validator on `dev/skills/build-aicadia`.
