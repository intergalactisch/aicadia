...
built), with `dev/docs/evidence/` as the bridge. ("Side" is deliberate vocabulary:
...
- `game/docs/` becomes an index plus per-concern and per-capability documents;
- delivery and evidence narratives collapse from every duplicated live site
  into one `dev/docs/evidence/` home, with only static pointers elsewhere — the
...
- superseded concept and research documents are archived with explicit
  ...
  ideas findable;
- `dev/docs/concept/10-discovery-and-world-context.md` (714 lines, five distinct
...
- the thirteen published tool descriptions move from Rust string constants into
  ...
  `game/mcp/agent/instruction.md`;
- the four code monoliths — `game/src/world.rs` (5,427 lines), `game/src/wire.rs`
...
`game/mcp/tool-catalog.json` is byte-identical, the published instructions
...
- Code decomposition is pure relocation: no renamed public item, no public
  ...
  digest-frozen runner scripts (`dev/playtest/agent/run`, `dev/playtest/trait/run`
  — except the two named single-line corrections in T7/T10), the single-page
  ledger (`web/index.html`) and `game/src/server.rs` (1,108 lines, within bounds)
  are deliberately not split.
- No change to completed or superseded `dev/plans/` history.

## Evidence and authorities

| Evidence or authority | Current fact | Consequence for this build |
| --- | --- | --- |
| `game/docs/README.md` (~1,000 lines) | Mixes contract, schema, validation, required evidence and ~40% delivery-status narrative | Decompose into index + role-pure files; delivery narrative moves to `dev/docs/evidence/` |
| `game/docs/agent-interface.md` (~800 lines) | Duplicates capability semantics against README; carries its own delivery-status block | Absorbed into `protocol.md`, `agent.md` and per-capability files |
| Trait delivery narrative | Repeated near-identically across at least seven live sites (game README, agent-interface, trait-playtest, agent-playtest, capability-map, the backlog README horizon row, the backlog item, concept 11, log) — the working tree keeps evolving, so T2's sweep, not a fixed count, defines the site list — now spanning three completed-as-failed paid candidates: `candidate-MmwRmcBv`, `candidate-ydttdFfc` and `candidate-63hjH4HW` | Written once in `dev/docs/evidence/trait.md`; every other live site becomes one static pointer |
| `dev/plans/20260814-111749-trait-live-validation/plan.md` | `status: complete` (accepted, spend-authorized and closed 2026-08-14); T5 review returned GO for the completed-as-failed claim; the sole known P1 (live `startswith("Pip ")` validator drift) is explicitly deferred; the log designates this architecture plan as the next chosen work with no active edge | This plan is rebased on that end state: no pending candidate exists, no other plan will freeze a digest, and the controller correction is a future plan, not a dependency here |
| `dev/docs/concept/00–09` | Pre-2026-08-07 generation; scene/claim model, persona dealing, MCP verb sketch all contradict the current system with no marker | Archive verbatim under `dev/docs/concept/archive/` with banners; still-live ideas stay indexed |
| `dev/docs/concept/log/log.md` (~2,180 lines) | Most entries sit under one stale `# 2026-08-10` heading covering work through 2026-08-14; the trait-live-validation opening and closure ARE logged in the working tree | Split into per-month files with restored real dates; future delivery detail lives in evidence |
| `dev/docs/research/README.md` | Index has no status column; two files are header-superseded; `ai-agent-graphs.md` describes a six-operation MVP that reads as current scope | Status/era columns; archive the three misleading files |
| `game/mcp/agent.rs:5-29` | Thirteen descriptions as Rust consts without trailing newlines (fixture check: zero descriptions end with `\n`) | Per-tool `.md` files each end with `\n`; `apply()` removes exactly one final newline via `strip_suffix('\n')` with an assert (never `trim_end`, which would silently eat meaningful trailing whitespace) |
| `src/agent-play-contract.txt` | One 52-line standalone text, already editable without touching Rust; published via `get_info()` (`game/src/server.rs:1072`) and injected by the adapter | Relocated whole to `game/mcp/agent/instruction.md`; not split, not reworded |
...
| `studio/tests/aicadia-local.sh:212` | Compares the recorded `developer_instructions=` argument against `jq -Rs . src/agent-play-contract.txt` | T7 updates the path in the same change |
| `game/tools/aicadia-agent:8,51-52,111-129` | Reads the contract from a hardcoded path behind an early exists/not-symlink guard and injects it as `developer_instructions` | T7 updates `CONTRACT=` to the new path; the guard and injection stay byte-identical |
| `dev/playtest/trait/run:1348` digest material | Candidate material is `find migration src -name '*.sql' -o -name '*.rs'` — Rust and SQL only; the contract text is not digest material today | T7 extends the predicate to `*.md`/`*.txt` under `src/` so published Agent text stays freeze material for any FUTURE live plan; with the validation plan closed and no candidate pending, digest drift from this plan blocks nothing |
| `dev/tests/trait-playtest.sh:167` | Digest-drift injection appends to `game/src/world.rs` specifically | T10 repoints it at a real post-split World source or the check goes silently vacuous |
...
| `game/docs/**` → `dev/plans/**` links | Six A→development authority links exist today (`README.md:19,69`, `agent-interface.md:8,53`, `agent-playtest.md:18`, `trait-playtest.md:150`); the two plan directories involved are still untracked — already broken on a fresh clone | Present defect; T2/T3 remove them, `dev/docs/evidence/trait.md` becomes the sole plan-pointer holder |
...
| `dev/skills/build-aicadia/SKILL.md` | 75 lines restating AGENTS.md rules; references `dev/docs/concept/log/log.md` and doc paths this plan moves | Owned by T1 (side map) and T9 (path alignment; broader duplication flagged, not resolved) |
...
| `AGENTS.md` "The MVP Is The Filter" | Enumerates the thirteen capability names and MVP shape, duplicating `game/docs/` | Slim to the rule plus a pointer; contract state lives only in `game/docs/` |
...
| User decisions this session | Markdown sources; no monolithic files; per-capability files; `dev/docs/evidence/` as own home; `AGENTS.md` stays one strictly formatted file; contract stays one file (former T8 discover-fetch dropped); code decomposition stays in this plan | Confirmed decisions below |
...
Governing authorities: `AGENTS.md` (build rules, amended by T1), `game/docs/`
(content redistributed, semantics unchanged), `dev/docs/concept/log/log.md` (records
...
`CLAUDE.md`), `game/mcp/agent.rs` plus new `game/mcp/agent/` text
sources, `game/src/world.rs` → `game/src/world/`, `game/src/wire.rs` → `game/src/wire/`,
`tests/world.rs` and `tests/server.rs` → directory test crates,
`game/tools/aicadia-agent` (path only), two single-line runner-script corrections,
...
- `dev/docs/evidence/` is its own top-level documentation home — product history is
  not planning state (`.agents/`) and not current contract (`game/docs/`). Its
...
- Outside `dev/docs/evidence/`, delivery status appears only as a STATIC pointer
  ("delivery history and current status: see `dev/docs/evidence/<slice>.md`") that
...
- `AGENTS.md` remains one file: at 262 lines it is not a monolith, it is the
  ...
  moves out.
- The concept log becomes per-month files under `dev/docs/concept/log/`; entries
...
- Sweep scope is split by check kind, because this plan and its move-map
  ...
  exclude all of `dev/plans/**` and the move-map fragments as explicit
...
- Each task updates references only inside its own owned tree and records
  ...
  (`dev/plans/20260814-130554-documentation-architecture/move-map/<task>.md`,
...
- Resolved OQ3 — User decision 2026-08-14, following the first review: the
  ...
  `game/mcp/agent/instruction.md`; only the thirteen tool descriptions
...
- Module boundaries inside `game/src/world/` (e.g. whether `read.rs`/`mutation.rs`
...
| --- | --- | --- | --- |
| `docs/README.md` | absent | New constitution structured as the side split — runtime side (src, migration, operations tooling, `game/docs/`) and development side (build rules, concept, research, planning, playtest machinery) with `dev/docs/evidence/` as the bridge — then the home table, the one-way reference rule, the frozen-history exemption (completed/superseded plans only), role-header convention and bounded-size rule; states explicitly that sides are roles, not directories | Constitution defines placement; it never restates content; it never reuses `World` for documentation roles |
| `AGENTS.md` | Duplicates MVP surface; no constitution anchor | Slim "The MVP Is The Filter" to rule + pointer; add one compact "One Home Per Truth" rule; extend "Every Choice Leaves A Trail" authority list and Reference Docs with `dev/docs/evidence/` and the constitution | All other heuristics byte-unchanged; file stays compact |
| `CLAUDE.md` | Points to AGENTS.md and concept | Add one line pointing to `docs/README.md` | Stays a thin pointer file |
| `dev/docs/evidence/` | absent (content scattered across 7 sites) | `README.md` index; `world-entry.md`, `action.md`, `interaction.md`, `property.md`, `trait.md` (all three paid Trait candidates, digests, audits, closure); `runner/agent-playtest.md`, `runner/trait-playtest.md` | Each narrative appears once; other sites carry one static pointer; no game-contract rules live here |
| `game/docs/README.md` | ~1,000-line monolith | Short index: authority statement, home map, reading order | Every removed fact lands in exactly one new home |
| `game/docs/domain.md` | part of README | Current domain model: User, Entity, Character, Place, Property, Trait, Activity semantics and role table | Semantics verbatim-preserved |
| `game/docs/protocol.md` | split across README/agent-interface | Request context, MCP revision/statelessness, wire conventions and shared shapes, pagination/cursors, canonical error table, delivery identity and Place freshness, HTTP statuses, origin rules | One cross-cutting home; capability files link instead of repeating |
| `game/docs/agent.md` | part of agent-interface | Host conformance, instruction hierarchy, player-mode communication, three workshop flows, and the text-layering rule (what lives in global instructions vs per-tool descriptions, and which redundancy is deliberate) | Layering rule governs the follow-up rewrite |
| `game/docs/capability/*.md` | absent | 13 files, fixed template: Purpose, Input, Validation, Result and Activity footprint, Errors, Workshop pointer, Evidence obligations + static evidence pointer | Names match the catalog exactly; no capability described elsewhere |
| `game/docs/storage.md` | part of README | PostgreSQL relations, indexes, locking, migration boundaries | Schema description matches migrations |
| `game/docs/deferred.md` | wall at README end | The explicit deferral list as an extensible file | Deferred means absent, per AGENTS.md |
| `game/docs/local-play.md` | current | Update links and the adapter paragraph (new contract path) | Role unchanged |
| `game/docs/agent-interface.md`, `game/docs/agent-playtest.md`, `game/docs/trait-playtest.md` | monoliths / evidence mix | Deleted after full absorption into `protocol.md`/`agent.md`/`capability/` and `dev/docs/evidence/` | `git log` retains history; every removal lands in the move map |
| `dev/docs/concept/README.md` | 36-line table | Guide: two generations, per-doc status, archive pointer, and a still-live-ideas index distinguishing superseded model choices from dormant-but-alive product ideas (steward, ripples/catch-up, naming economy, sealed envelopes, tension sources, anti-patterns) | Concept never governs implementation |
| `dev/docs/concept/10-discovery-and-world-context.md` | 714-line record spanning five themes | Split into themed live records: `discovery.md`, `knowledge.md`, `interaction.md`, `spatial.md`, `time.md`, `tabletop.md`; delivered Interaction content becomes rationale + pointer to `game/docs/` | Every open decision and frontier item keeps exactly one home; the prototype HTML stays beside `discovery.md`; all renames land in the move map |
| `dev/docs/concept/11-entity-traits-and-change.md` | duplicates delivery saga | Trimmed to `entity-state.md`: unique concept rationale kept; delivery detail becomes one static pointer | Concept rationale preserved; rename in the move map |
| `dev/docs/concept/log/` | one ~2,180-line file | `README.md` (from the current preamble) + `2026-07.md`, `2026-08.md`; real-date headings restored inside the August block | Entries regrouped, never reworded; append-only discipline stated in the index |
| `dev/docs/research/README.md` | question-only index | Add Status (load-bearing / pending / historical / superseded) and Era columns; archived files keep an index row pointing into `archive/` — the README is the one index, no separate archive index exists | Research informs, never decides |
| `dev/docs/research/archive/` | absent | `open-spatial-world-system.md`, `hierarchical-spatial-model.md`, `ai-agent-graphs.md` moved with banners; July-era vocabulary banner added to remaining July files | Files verbatim below the banner; moves land in the move map |
| `game/mcp/agent.rs` | consts + include of one txt | Per-tool `include_str!` table; `apply()` removes exactly one final newline per description via `strip_suffix('\n')` + assert; instruction include repointed to `instruction.md`; all existing tests kept | Published bytes identical; startup assert kept; no description ends with `\n` |
| `game/mcp/agent/instruction.md` | absent (`src/agent-play-contract.txt` today) | `git mv`, byte-identical; inbound paths updated in `agent_contract.rs`, `tests/server.rs:27`, `studio/tests/aicadia-local.sh:212`, `game/tools/aicadia-agent` (`CONTRACT=`, guard kept at the same early position) | Bytes unchanged; every consumer repointed in the same task |
| `game/mcp/agent/tool/*.md` | absent | Thirteen description files, byte-exact | `game/mcp/tool-catalog.json` unchanged |
| `game/mcp/agent/README.md` | absent | States the bytes are published verbatim to every Agent and formatting is load-bearing; "fixing the Markdown" is a contract change requiring the rewrite plan | Guards against later markdown-fixing that would break pinned phrases |
| `dev/playtest/trait/run:1348` | candidate material `*.sql`/`*.rs` only | Predicate extended with `*.md`/`*.txt` under `src/` | Published Agent text becomes freeze material for future live plans |
| `game/src/world.rs` | 5,427-line monolith: `World` impl (16 operations), private Property/Trait persistence machinery, domain types, errors | Directory module `game/src/world/`: `model.rs` (ids + domain types), `error.rs`, `property.rs`, `entity_trait.rs` (`trait` is a Rust keyword), `activity.rs`, operation impl blocks grouped by concern (`read.rs`, `mutation.rs`); `mod.rs` keeps `World` and re-exports | Public seam `World` unchanged; `lib.rs` re-exports identical; pure moves |
| `game/src/wire.rs` | 2,156 lines of ~30 input/output types | Directory module `game/src/wire/` split by concern with unchanged re-exports and every serde/schemars/utoipa attribute identical | Serialized JSON shapes byte-identical; OpenAPI/MCP schemas unchanged |
...
| `dev/tests/trait-playtest.sh:167` | drift injection appends to `game/src/world.rs` | Repointed at a real post-split World source (e.g. `game/src/world/mutation.rs`) | The drift check keeps proving World-source coverage |
| `dev/backlog/capability-map.md` | stale delivery bullets | Trait bullet and delivery block become one static evidence pointer | Map stays a living index, not an authority |
| `dev/backlog/items/entity-trait-development.md` | delivery narrative | Item keeps its own planning state (`Done`) plus one static evidence pointer | Item states outcome, not saga |
| `dev/skills/build-aicadia/SKILL.md` | references paths this plan moves | Paths repointed via the move map in T9 (`:14-18,63,69`); broader AGENTS.md duplication flagged as follow-up candidate, not resolved here | Skill keeps working during and after the move |
| Move map (plan directory) | absent | Directory `move-map/` with one fragment per task (`t2.md` … `t10.md`): every old path/anchor → new path/heading, written only by its owning task | T9 concatenates the fragments; its cross-tree pass is driven exclusively by them and searches old path TOKENS as plain text, not only Markdown links; the fragments are validated separately (one destination per old source, every destination exists) and are, like all of `dev/plans/**`, exempt from the token scan |
...
| T2 | completed | T1 | no | Evidence home extraction | `dev/docs/evidence/**`, static-pointer replacements in every live delivery site its sweep finds (known: game README, agent-interface, both playtest docs, capability-map, backlog README horizon row, backlog item, concept 11), `move-map/t2.md` | Each narrative greps to exactly one home; pointers restate no status |
| T3 | completed | T2 | no | Game contract decomposition + anchor map | `game/docs/**`, `move-map/t3.md` | Fact-coverage review; in-tree links resolve; no game doc > 400 lines |
| T4 | completed | T1, T2 | no | Concept archive, doc 10 themed split, doc 11 trim | `dev/docs/concept/**` except `log/`, `move-map/t4.md` | Banners present; archived content verbatim; every doc-10 fact has one themed home |
| T5 | completed | T1 | yes | Log restore + split | `dev/docs/concept/log/**`, `move-map/t5.md` | Heading/entry mapping table complete; entry text unchanged |
| T6 | completed | T1 | yes | Research status index + archive | `dev/docs/research/**` (index/banners/moves only, no link edits), `move-map/t6.md` | Index rows cover every live and archived file; 3 files archived |
| T7 | completed | T1 | yes | Agent-text relocation: tool split + contract move | `game/mcp/agent.rs`, `game/mcp/agent/**`, `src/agent-play-contract.txt`, `tests/server.rs` (include path), `tests/world.rs` (one baseline planner assertion), `studio/tests/aicadia-local.sh` (path), `game/tools/aicadia-agent` (`CONTRACT=`), `dev/playtest/trait/run` (predicate line), `move-map/t7.md` | `cargo test` green incl. discover assertions; fixture diff empty; no description ends with `\n`; `studio/tests/aicadia-local.sh` passes |
| T10 | completed | T7 | no | Code decomposition by pure moves | `game/src/world.rs` → `game/src/world/**`, `game/src/wire.rs` → `game/src/wire/**`, `tests/world.rs` → `tests/world/**`, `tests/server.rs` → `tests/server/**`, `src/lib.rs`, `dev/tests/trait-playtest.sh` (drift line + hermetic candidate-state gates), `move-map/t10.md` | One-to-one test mapping with equal counts, all green; fixture diff empty; drift line targets a real file |
| T9 | completed | T3,T4,T5,T6,T7,T10 | no | Cross-tree link pass + integrity sweep + alignment | whole tree, `dev/backlog/*`, `dev/skills/build-aicadia/SKILL.md` | Validation ladder passes |
...
1. Write `docs/README.md` with the side split as its top-level structure:
   **runtime side — the running product** (`src/` including
   `game/mcp/agent/`, `migration/`, app test crates under `tests/`,
   `game/docs/`, `studio/tools/aicadia-local`, `game/tools/aicadia-agent`, `web/`) and
   **development side — development of the product** (`AGENTS.md`,
   `CLAUDE.md`, `dev/CONTEXT.md`, `dev/docs/concept/` + log + archive,
   `dev/docs/research/` + archive, `dev/backlog/`, `dev/plans/`,
   `dev/skills/`, `tools/*-playtest`, `tests/*-playtest.sh`), with
   `dev/docs/evidence/` as the bridge (development history about the product,
   ...
   `dev/docs/evidence/`; the split sweep scope: frozen history (log entries,
   ...
   all of `dev/plans/**` and move-map fragments as explicit planning
   citations;
...
2. Amend `AGENTS.md` exactly three ways: slim "The MVP Is The Filter" to the
   rule plus a pointer to `game/docs/` for the current surface; add compact
   rule "One Home Per Truth" anchoring `docs/README.md`; extend "Every Choice
   Leaves A Trail" and Reference Docs with `dev/docs/evidence/` and the
   constitution.
...
- Stop if slimming "The MVP Is The Filter" would remove a constraint not present
  in `game/docs/` — that content must first gain its home.

### T2 — Evidence home extraction

**Objective:** Every delivery/evidence narrative exists exactly once under
`dev/docs/evidence/`; all live sites carry one static pointer.

**Actions:**

1. Create `dev/docs/evidence/README.md` (role header; index: slice → current status →
...
3. Move runner operation contracts to `dev/docs/evidence/runner/agent-playtest.md`
   and `dev/docs/evidence/runner/trait-playtest.md`, trimming their duplicated
...
4. Find every live delivery site with the sweep itself — `grep -rn
   ...
   STATIC pointer (no restated status). Known sites: `game/docs/README.md`,
   `game/docs/agent-interface.md`, `dev/backlog/capability-map.md`, the
   `dev/backlog/README.md` horizon row for entity-trait-development
   (the row keeps its own planning state `Done` plus the pointer),
   `dev/backlog/items/entity-trait-development.md` and
   `dev/docs/concept/11-entity-traits-and-change.md` — but the sweep, not this
   list, defines completeness. The six `game/docs/**` → `dev/plans/**`
   authority links disappear here or in T3; `dev/docs/evidence/trait.md` becomes
   the sole plan-pointer holder.
5. Append every removed/moved file and heading to `move-map/t2.md`. Sweep
   `grep -rn "game/docs/agent-playtest\|game/docs/trait-playtest" tools tests
...
- `dev/tests/trait-playtest.sh` and `dev/tests/agent-playtest.sh` still pass.
...
**Objective:** `game/docs/` is an index plus role-pure files; every fact from the
...
3. Rewrite `game/docs/README.md` as the short index with reading order; update
   links WITHIN `game/docs/`.
...
- In-tree link sweep: every relative link and anchor WITHIN `game/docs/`
  resolves.
- `wc -l game/docs/*.md game/docs/capability/*.md` — no file over 400 lines.
...
1. Move `00-vision.md` … `09-world-graph.md` to `dev/docs/concept/archive/`
...
2. Split `10-discovery-and-world-context.md` into themed live records —
   ...
   delivered parts become pointer to `game/docs/`), `spatial.md`, `time.md`,
...
3. Trim doc 11 to `entity-state.md`: keep the Property/Trait domain-distinction
   ...
   T2 placed.
4. Rewrite `dev/docs/concept/README.md` as the guide: live themed records, per-doc
   ...
   product idea.
5. Update links WITHIN `dev/docs/concept/` (including
...
- `dev/docs/concept/` still cannot override `game/docs/` (restate in README).

**Evidence:**

- `diff` of each archived file against `git show HEAD:dev/docs/concept/<name>` —
  banner-only delta.
...
1. Split `log.md` into `dev/docs/concept/log/2026-07.md` and
   `dev/docs/concept/log/2026-08.md`; the current preamble (title, tag vocabulary,
   separator) becomes `dev/docs/concept/log/README.md` (role header, consolidated
   tag vocabulary including the ad-hoc tags that appeared, append-only rule,
   and the new rule that delivery bookkeeping lives in `dev/docs/evidence/` with
...
1. Add Status (load-bearing / pending / historical / superseded) and Era
   ...
   `dev/docs/research/README.md`, using the completed research audit. Archived
...
2. Move `open-spatial-world-system.md`, `hierarchical-spatial-model.md` and
   `ai-agent-graphs.md` to `dev/docs/research/archive/` with supersession banners
...
the play contract lives whole at `game/mcp/agent/instruction.md`; published
bytes are unchanged.

**Actions:**

1. Create `game/mcp/agent/tool/<tool_name>.md` (13), each containing
   ...
   drift.
2. `git mv src/agent-play-contract.txt game/mcp/agent/instruction.md`
   (byte-identical) and update the four inbound paths in the same change:
   the `include_str!` in `game/mcp/agent.rs`, the include at
   `tests/server.rs:27`, the comparison path at `studio/tests/aicadia-local.sh:212`,
   and `CONTRACT=` in `game/tools/aicadia-agent` — keeping the existing
   exists/not-symlink guard at the same early position.
3. Add `game/mcp/agent/README.md`: these bytes are published verbatim to
...
4. Extend the `dev/playtest/trait/run` candidate-material predicate
...
- `game/mcp/tool-catalog.json` byte-identical; `cargo test` fully green,
...
- `git diff --stat game/mcp/tool-catalog.json` empty; `cargo test` green;
  `jq '[.[] | select(.description | endswith("\n"))] | length'
  game/mcp/tool-catalog.json` returns 0; `studio/tests/aicadia-local.sh` passes.
...
2. Convert `game/src/world.rs` into directory module `game/src/world/`: `model.rs` (id
   ...
   Moves only.
3. Convert `game/src/wire.rs` into directory module `game/src/wire/` (`input.rs`,
...
6. Repoint the digest-drift injection `dev/tests/trait-playtest.sh:167`
   (`printf … >> "$copy/game/src/world.rs"`) at a real post-split World source such
   as `$copy/game/src/world/mutation.rs` — otherwise the check appends to a
   ...
   untouched.
7. Append the four old-path → new-module mappings (`game/src/world.rs`,
   `game/src/wire.rs`, `tests/world.rs`, `tests/server.rs`) to `move-map/t10.md` —
   live flat references to these paths exist (e.g.
   `dev/docs/research/character-entity-control-model.md:511`,
   `dev/backlog/items/world-entry-history.md:145`) and T9 repoints them.

**Invariants:**

- No renamed public item, public signature, SQL string, dependency or behavior
  change; no new abstraction; `game/src/server.rs`, the runner scripts (beyond the
...
- `git diff --stat game/mcp/tool-catalog.json` empty;
  `dev/tests/trait-playtest.sh` passes AND the drift-injection line provably
...
2. Concatenate the fragments and execute the single cross-tree link pass:
   repoint every live reference in `docs/`, `dev/backlog/**`,
   `dev/skills/build-aicadia/SKILL.md` (`:14-18,63,69`), root files and
   any draft/active plan whose links are meant to resolve. Search every old
   path as a plain-text TOKEN (e.g. `tests/world.rs`, `game/src/world.rs`,
...
4. Old-token scan: outside `dev/plans/**`, the log, archives and the
...
5. Duplication greps: candidate ids, digests, the thirteen-name enumeration
   (allowed only in code, fixture, `game/docs/capability/` filenames and the
   game index), delivery-status phrases — excluding `dev/plans/**`
   (explicit planning citations), the log and archives.
6. Reference-direction sweep: no runtime-side file (`game/docs/**`, `src/**`,
   `studio/tools/aicadia-local`, `game/tools/aicadia-agent`, `web/`) links to
   `dev/docs/concept/`, `dev/docs/research/` or `.agents/` as authority; the only
   runtime→development links are static pointers into `dev/docs/evidence/`.
7. Monolith audit: `wc -l` over current-authority docs and source modules
   against the bounded-size rule and its exemptions.
8. Align `dev/backlog/README.md` wording and capability-map links; flag —
...
2. **Contract:** `cargo test --all-targets` green; `game/mcp/tool-catalog.json`
   byte-identical; `studio/tests/aicadia-local.sh`, `dev/tests/agent-playtest.sh`,
   `dev/tests/trait-playtest.sh` pass; a live `tools/list` + `server/discover`
...
3. **Outcome:** one status change rehearsal — editing the Trait status touches
   exactly one file (`dev/docs/evidence/trait.md`), because every other site holds
   a static pointer; one tool-description iteration rehearsal — editing
   `game/mcp/agent/tool/submit_action.md` touches no Rust; one code
   navigation rehearsal — the Property write path is findable from
   `game/src/world/property.rs` alone; a new reader can answer "where does X live?"
...
- Reference direction and placement pass: runtime-to-concept/research/planning
  ...
  no bounded game authority exceeds 400 (largest: `game/docs/protocol.md`, 399).
...
- `studio/tests/aicadia-local.sh`, `dev/tests/agent-playtest.sh` and
  `dev/tests/trait-playtest.sh` pass. A disposable locally started server returned 13
...
- Disposable outcome rehearsals prove a Trait-status edit touches only
  `dev/docs/evidence/trait.md`, a `submit_action.md` description edit touches no Rust,
  the Property write path is navigable from `game/src/world/property.rs`, and
...
- `AGENTS.md`, `CLAUDE.md`, `dev/CONTEXT.md`, `docs/`, `dev/backlog/` and the
