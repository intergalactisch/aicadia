...
| --- | --- | --- |
| `game/docs/README.md` | `create_entity` creates an unplaced Entity; Activity has actor/context/involved ids but no prose | Deliberately evolve the contract with one compound action while preserving granular Entity introduction |
| `game/docs/README.md` Character and entry | World already derives one Character and nullable exact current Place from User context | Action and local reads accept no User, Character or Place selectors and reject an unplaced Character |
| `game/docs/agent-interface.md` | Ten HTTP/MCP capabilities share one World contract; MCP initialization carries global Agent instructions | Ship action and local reads through World, HTTP and MCP and publish the private-workshop flow in Agent instructions |
| `game/src/world.rs` | Every mutation locks User context, writes state and Activity in one Postgres transaction | Deepen this existing World seam; Agent and adapters never receive storage authority |
...
| --- | --- | --- | --- |
| `game/docs/README.md` | Governs the implemented first action, prose, placement, local reads, retry and errors | Freeze; change only if evidence disproves current behavior | Current state remains authoritative; no event sourcing or server intelligence |
| `game/docs/agent-interface.md` | Publishes all thirteen capabilities and the private action workflow | Freeze | User/Character/Place ids remain derived where contextual |
...
| `game/src/world.rs`, `src/lib.rs` | Implement and export the accepted action, local reads, validation and transactions | Freeze | World alone writes; all-or-nothing; exact roles and context |
| `game/src/wire.rs` | Implements strict shared wire types, typed cursors, Place revision and error mappings | Freeze | Unknown fields rejected; adapters share semantics |
| `game/src/server.rs` | Implements thirteen thin HTTP/MCP capabilities and action guidance | Freeze | Complete catalog parity; no adapter orchestration |
...
| `tests/server.rs`, `game/mcp/tool-catalog.json` | Nine passing server tests and exact thirteen-tool fixture prove adapter parity | Re-run unchanged in T4R2 | Published Agent contract is complete and deterministic |
| `dev/playtest/agent/run`, `dev/playtest/agent/schema/*`, `dev/tests/agent-playtest.sh` | Minimal observer, HTTP-before-observer order, separate phase results and exact Activity-actor comparison passed token-free and live evidence | Freeze until a new concrete claim earns a change | No product/API change; no fourth observer tool; fake evidence can never become live evidence; no automatic run |
| `game/docs/agent-playtest.md` | Records both earlier candidates, the accepted protocol and independently verified `run-gE8iED5m` | Freeze | Past manifests and evidence claims are immutable history |
| `dev/docs/concept/log/log.md`, `dev/backlog/` | Record accepted build choices, evidence corrections and completed outcome | Freeze; a next edge requires separate selection | Planning does not override `game/docs/` |
...
| --- | --- | --- | --- | --- | --- | --- |
| T1 | completed | — | no | Install the accepted behavior and Agent contract | `game/docs/*`, concept log, backlog, plan | Every confirmed actor/action/state/history/error boundary is explicit |
| T2 | completed | T1 | no | Implement atomic World state, history and revisioned local reads | migration, `game/src/world.rs`, `src/lib.rs`, `tests/world.rs` | Focused World tests prove acceptance, rollback, retry, Place freshness and cross-User local state |
| T3 | completed | T2 | no | Ship HTTP/MCP parity and Agent guidance | `game/src/wire.rs`, `game/src/server.rs`, `tests/server.rs`, catalog fixture | Both adapters expose identical action and read semantics |
| T4R1 | completed | T3 | no | Make every live evidence layer minimal, observable and independently retained | `dev/playtest/agent/run`, `dev/playtest/agent/schema/*`, `dev/tests/agent-playtest.sh`, `game/docs/agent-playtest.md` | Live-shaped fake paths, public preflight and wrong-actor regression pass token-free; zero model calls and leftover databases |
...
| T4R3 | completed | T4R2 + fresh explicit authorization | no | Run exactly one continuous live candidate | private `dev/.playtest/agent/run-gE8iED5m/` evidence and its owned disposable database | Runner reports proposal, preview, commit, HTTP, observer and cleanup passed continuously; no retry occurred |
| T4R4 | completed | T4R3 | no | Reconcile the exact outcome and close or retain the edge | plan, `game/docs/agent-playtest.md`, concept log, backlog | Independent raw review GO; authorities aligned, backlog Done and zero leftovers |
...
2. Amend `game/docs/README.md` and `game/docs/agent-interface.md` with exact domain,
...
- `game/docs/` is the only current executable truth.
...
5. Align `game/docs/agent-playtest.md` to the accepted order and keep both earlier run
...
- `/bin/bash dev/tests/agent-playtest.sh` — live-shaped success and fail-closed paths pass
  without a model call or persistent database.
- `DATABASE_URL=postgres://localhost:5433/postgres dev/playtest/agent/run preflight` —
...
- `/bin/bash -n dev/playtest/agent/run dev/tests/agent-playtest.sh`
- `/bin/bash dev/tests/agent-playtest.sh`
- `git diff --check`
...
2. Run exactly
   `DATABASE_URL=postgres://localhost:5433/postgres dev/playtest/agent/run run --confirm-token-spend`
...
1. If T4R3 passed every gate, record the retained run id and bounded claim in
   `game/docs/agent-playtest.md`, record the accepted evidence/operational choices in
