...
| --- | --- | --- |
| `game/docs/local-play.md` | The stable local profile and current Agent contract are the supported local authority. | Reuse the running World and existing User; do not provision or reset. |
| `game/docs/agent-interface.md` | A conforming host requires stateless `2026-07-28`, discovery instructions, the complete catalog and fail-closed live authority. | Verification must inspect those exact facts. |
...
| `dev/.local/profile.json` and live port check | Database `aicadia_local`, stable User `f149fb45-9be1-494e-b2b7-79a56c39475e`, and loopback port 3000 are available. | Supply that UUID to the desktop process without writing it into the tracked repository. |
| `dev/docs/research/chatgpt-local-and-hosted-mcp-access.md` | Stateless-only is confirmed and a skill is deferred until a demonstrated workflow gap. | Do not broaden this experiment. |
