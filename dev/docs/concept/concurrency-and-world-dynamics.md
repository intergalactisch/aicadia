...
- `dev/lab/` is a lasting general experimental workbench, with multiplayer as its first
  current track under `dev/lab/multiplayer/`;
...
and the material choice is appended to `dev/docs/concept/log/`. New sourced findings and
their citations live in `dev/docs/research/`; only later accepted current game behavior
lives in `game/docs/`. `dev/lab/` remains separate: it contains experimental realizations
...
The retained
[`observation-ownership` experiment](../../../dev/lab/multiplayer/01-observation-ownership/README.md)
...
notifications remain explicitly deferred in `game/docs/`. Accepting an SSE response
...
`dev/lab/multiplayer/02-subject-conflict` passed ten fixed interleavings: independent
...
`dev/lab/multiplayer/03-postgres-subject-conflict` deliberately refuted a stronger
...
`dev/lab/multiplayer/04-postgres-conflict-strategies` fixture therefore held one
