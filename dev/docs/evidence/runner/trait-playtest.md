...
> **Excludes:** game-contract rules and delivery status; see `game/docs/` and `dev/docs/evidence/trait.md`.
...
```sh
DATABASE_URL='postgres://localhost/postgres' dev/playtest/trait/run preflight
```
...
Preflight and candidate evidence live beneath `dev/.playtest/trait/`. The root
...
[`dev/playtest/agent/run`](../../../../dev/playtest/agent/run) is now the current bounded
