...
> **Excludes:** current entry behavior and runner operations; see `game/docs/` and `runner/agent-playtest.md`.

## Deterministic completion record

- `migration/0004_world_entry_activity.sql`, `game/src/world/`, `game/src/wire/` and
  `game/src/server.rs` implement the accepted state, transaction and adapter contract.
...
- `game/docs/`, `dev/CONTEXT.md`, `dev/docs/concept/log/README.md`, `AGENTS.md` and the Agent
...
the Bash syntax checks, `bash dev/tests/agent-playtest.sh`, `git diff --check` and the real
