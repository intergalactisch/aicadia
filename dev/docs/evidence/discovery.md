...
> **Excludes:** current game rules, decision rationale and runner operation; see `game/docs/`, `dev/docs/concept/discovery.md` and `runner/agent-playtest.md`.
...
The [current game contract](../../../game/docs/README.md) owns the executable rules. Migration
`0010_investigation.sql`, `game/src/world/investigation/`, `game/src/wire/investigation.rs` and
`game/src/server/` implement them. The server split leaves HTTP and MCP as private thin
...
`studio/tests/aicadia-local.sh` separately proves real launcher persistence. It creates an
...
- `bash studio/tests/aicadia-local.sh`, including
  `restart_investigation_retry_identical=true` and `codex_invoked=false`;
- `bash dev/tests/agent-playtest.sh`;
- `bash dev/tests/trait-playtest.sh`, including owned-database failure cleanup;
...
plan](../../../dev/plans/20260815-180754-discovery-review-corrections/plan.md)
...
- the investigation result field `limits` became singular `limit`; the wire
  ...
  are proved on both adapters; `game/src/server/mod.rs` is composition-only;
...
(51 library, 2 database-helper, 15 server, 84 World), `bash dev/tests/agent-playtest.sh`,
`bash dev/tests/trait-playtest.sh` (after refreezing the token-free candidate digest for
the changed sources) and `bash studio/tests/aicadia-local.sh`. The catalog fixture changed
